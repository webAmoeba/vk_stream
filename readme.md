...
- STREAM_MODE="auto"          # auto | concat | file (auto → file при STREAM_SUB_BURN=1)
...
- В concat‑режиме для прожига субтитров используется отдельный плейлист `vk_stream_subs_playlist.txt` в VIDEO_DIR, он перезаписывается при каждом запуске.
- При STREAM_SUB_BURN=1 используется покадровый (file) режим: каждый файл идёт отдельным FFmpeg, возможна короткая пауза 1–3 сек между файлами. Это сделано для стабильной работы субтитров.
